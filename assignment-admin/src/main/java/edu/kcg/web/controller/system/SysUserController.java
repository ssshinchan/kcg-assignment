...
import edu.kcg.system.service.ISysPostService;
...
public class SysUserController extends BaseController
{
    ...
    public void export(HttpServletResponse response, SysUser user)
    {
        ...
        util.exportExcel(response, list, "ユーザーデータ");
    }
    ...
    {
        ExcelUtil<SysUser> util = new ExcelUtil<SysUser>(SysUser.class);
        util.importTemplateExcel(response, "ユーザーデータ");
    }
    ...
    public AjaxResult add(@Validated @RequestBody SysUser user)
    {
        ...
        {
            return error("ユーザーの新規追加 '" + user.getUserName() + "' に失敗しました。ログインアカウントが既に存在します");
        }
        else if (StringUtils.isNotEmpty(user.getPhonenumber()) && !userService.checkPhoneUnique(user))
        {
            return error("ユーザーの新規追加 '" + user.getUserName() + "' に失敗しました。携帯電話番号が既に存在します");
        }
        else if (StringUtils.isNotEmpty(user.getEmail()) && !userService.checkEmailUnique(user))
        {
            return error("ユーザーの新規追加 '" + user.getUserName() + "' に失敗しました。メールアドレスが既に存在します");
        }
    ...
    public AjaxResult edit(@Validated @RequestBody SysUser user)
    {
        ...
        if (!userService.checkUserNameUnique(user))
        {
            return error("ユーザーの変更 '" + user.getUserName() + "' に失敗しました。ログインアカウントが既に存在します");
        }
        else if (StringUtils.isNotEmpty(user.getPhonenumber()) && !userService.checkPhoneUnique(user))
        {
            return error("ユーザーの変更 '" + user.getUserName() + "' に失敗しました。携帯電話番号が既に存在します");
        }
        else if (StringUtils.isNotEmpty(user.getEmail()) && !userService.checkEmailUnique(user))
        {
            return error("ユーザーの変更 '" + user.getUserName() + "' に失敗しました。メールアドレスが既に存在します");
        }
