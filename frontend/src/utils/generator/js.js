  ...
  if (trigger[conf.tag]) {
    if (conf.required) {
      ...
      let message = Array.isArray(conf.defaultValue)
        ? `少なくとも 1 つ選択してください${conf.vModel}`
        : conf.placeholder
      if (message === undefined) message = `${conf.label}空にすることはできません`
      rules.push(
...
function buildBeforeUpload(conf) {
  ...
  if (conf.fileSize) {
    ...
    if(!isRightSize){
      proxy.$modal.msgError('ファイルサイズが超過しています ${conf.fileSize}${conf.sizeUnit}')
    }`
  ...
  if (conf.accept) {
    ...
    if(!isAccept){
      proxy.$modal.msgError('選ぶべきだ${conf.accept}ファイルの種類')
    }`
