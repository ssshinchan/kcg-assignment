  ...
  <div class="header-search">
    ...
    >
      <el-input
        ...
        @input="querySearch"
        prefix-icon="Search"
        placeholder="メニュー検索、タイトルと URL のあいまいクエリをサポート"
        clearable
      ...
      <div class="result-count" v-if="search && options.length > 0">
        見つかりました <strong>{{ options.length }}</strong> 結果
      </div>
      ...
      <div class="result-wrap">
        <el-scrollbar>
          ...
          <div class="empty-state" v-else-if="search && options.length === 0">
            ...
            <p class="empty-text">見つかりません "<strong>{{ search }}</strong>" 関連メニュー</p>
            <p class="empty-tip">他のキーワードまたはパスを試してください</p>
          </div>
