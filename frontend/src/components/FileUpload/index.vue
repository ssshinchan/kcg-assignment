<template>
  <div class="upload-file">
    ...
    <div class="el-upload__tip" v-if="showTip && !disabled">
      アップロードしてください
      <template v-if="fileSize"> サイズを超えない <b style="color: #f56c6c">{{ fileSize }}MB</b> </template>
    ...
    <transition-group ref="uploadFileList" class="upload-file-list el-upload-list el-upload-list--text" name="el-fade-in-linear" tag="ul">
      <li :key="file.uid" class="el-upload-list__item ele-upload-list__item-content" v-for="(file, index) in fileList">
        ...
        <div class="ele-upload-list__item-content-action">
          <el-link underline="never" @click="handleDelete(index)" type="danger" v-if="!disabled">&nbsp;削除</el-link>
        </div>
      </li>
    </transition-group>
  </div>
...
function handleBeforeUpload(file) {
  ...
  if (props.fileSize) {
    ...
    if (!isLt) {
      proxy.$modal.msgError(`アップロードするファイルのサイズは以下を超えることはできません ${props.fileSize} MB!`)
      return false
    }
  }
  proxy.$modal.loading("ファイルをアップロードしています。お待ちください。...")
  number.value++
  return true
}
...
function handleUploadError(err) {
  proxy.$modal.msgError("ファイルのアップロードに失敗しました")
