  ...
  <div class="app-container">
    <el-row :gutter="10">
      ...
      <el-col :span="8">
        <el-card style="height: calc(100vh - 125px)">
          <template #header>
            <Key style="width: 1em; height: 1em; vertical-align: middle;" /> <span style="vertical-align: middle;">キーリスト</span>
            <el-button
              style="float: right; padding: 3px 0"
              link
              type="primary"
...
function refreshCacheNames() {
  getCacheNames()
  proxy.$modal.msgSuccess("キャッシュリストが正常に更新されました")
}

/** 清理指定名称缓存 */
function handleClearCacheName(row) {
  clearCacheName(row.cacheName).then(response => {
    proxy.$modal.msgSuccess("キャッシュされた名前をクリアする[" + row.cacheName + "]成功")
    getCacheKeys()
  })
}
...
function refreshCacheKeys() {
  getCacheKeys()
  proxy.$modal.msgSuccess("キーリストが正常に更新されました")
}

/** 清理指定键名缓存 */
function handleClearCacheKey(cacheKey) {
  clearCacheKey(cacheKey).then(response => {
    proxy.$modal.msgSuccess("キャッシュキーをクリアする[" + cacheKey + "]成功")
    getCacheKeys()
  })
}
...
function handleClearCacheAll() {
  clearCacheAll().then(response => {
    proxy.$modal.msgSuccess("すべてのキャッシュが正常にクリアされました")
  })
}
