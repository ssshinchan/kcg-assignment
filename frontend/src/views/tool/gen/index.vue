<template>
  <div class="app-container">
    ...
    <el-table ref="genRef" v-loading="loading" :data="tableList" @selection-change="handleSelectionChange" :default-sort="defaultSort" @sort-change="handleSortChange">
      ...
      <el-table-column label="シリアル番号" type="index" width="50" align="center">
        <template #default="scope">
          <span>{{(queryParams.pageNum - 1) * queryParams.pageSize + scope.$index + 1}}</span>
        </template>
      </el-table-column>
      <el-table-column label="テーブル名" align="center" prop="tableName" :show-overflow-tooltip="true" />
      <el-table-column label="テーブルの説明" align="center" prop="tableComment" :show-overflow-tooltip="true" />
      <el-table-column label="エンティティ" align="center" prop="className" :show-overflow-tooltip="true" />
      <el-table-column label="作成時間" align="center" prop="createTime" width="160" sortable="custom" :sort-orders="['descending', 'ascending']" />
      ...
      <el-table-column label="操作" align="center" width="330" class-name="small-padding fixed-width">
        <template #default="scope">
          ...
          <el-tooltip content="同期する" placement="top">
            <el-button link type="primary" icon="Refresh" @click="handleSynchDb(scope.row)" v-hasPermi="['tool:gen:edit']"></el-button>
          </el-tooltip>
          <el-tooltip content="コードを生成する" placement="top">
...
function handleSynchDb(row) {
  ...
  proxy.$modal.confirm('同期を強制することを確認します"' + tableName + '"テーブル構造?').then(function () {
    return synchDb(tableName)
  }).then(() => {
    proxy.$modal.msgSuccess("同期が成功しました")
  }).catch(() => {})
}
...
function copyTextSuccess() {
  proxy.$modal.msgSuccess("正常にコピーされました")
}
...
function handleEditTable(row) {
  ...
  proxy.$tab.openPage("変更[" + tableName + "]構成の生成", '/tool/gen-edit/index/' + tableId, params)
}

/** 删除按钮操作 */
function handleDelete(row) {
  const tableIds = row.tableId || ids.value
  proxy.$modal.confirm('削除テーブル番号が正しいか確認しますか?"' + tableIds + '"データ項目?').then(function () {
