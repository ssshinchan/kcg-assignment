    ...
    <el-tabs v-model="activeName">
      ...
      <el-tab-pane label="フィールド情報" name="columnInfo">
        <el-table ref="dragTable" :data="columns" row-key="columnId" :max-height="tableHeight">
          <el-table-column label="シリアル番号" type="index" min-width="5%" class-name="allowDrag"/>
          <el-table-column label="フィールド列名" prop="columnName" min-width="10%" :show-overflow-tooltip="true" class-name="allowDrag"/>
          <el-table-column label="フィールドの説明" min-width="10%">
          ...
          />
          <el-table-column label="Javaの型" min-width="11%">
            <template #default="scope">
          ...
          <el-table-column label="Javaのプロパティ" min-width="10%">
            <template #default="scope">
          ...
          <el-table-column label="クエリ" min-width="5%">
            <template #default="scope">
              <el-checkbox true-value="1" false-value="0" v-model="scope.row.isQuery"></el-checkbox>
            </template>
          </el-table-column>
          <el-table-column label="クエリ方法" min-width="10%">
            <template #default="scope">
          ...
          <el-table-column label="表示タイプ" min-width="12%">
            <template #default="scope">
              <el-select v-model="scope.row.htmlType">
                ...
                <el-option label="ラジオボタン" value="radio" />
                <el-option label="チェックボックス" value="checkbox" />
                <el-option label="日付管理" value="datetime" />
                <el-option label="画像のアップロード" value="imageUpload" />
                <el-option label="ファイルのアップロード" value="fileUpload" />
                <el-option label="リッチテキストコントロール" value="editor" />
              </el-select>
            </template>
          </el-table-column>
      ...
      <el-tab-pane label="情報の生成" name="genInfo">
...
function submitForm() {
  ...
  Promise.all([basicForm, genForm].map(getFormPromise)).then(res => {
    ...
    if (validateResult) {
      ...
      proxy.$modal.msgError("フォームの認証に失敗しました。送信内容を再確認してください。")
    }
  })
}
