<template>
  <div class="container">
      ...
      <el-scrollbar class="left-scrollbar">
        <div class="components-list">
          <div class="components-title">
            <svg-icon icon-class="component" />入力コンポーネント
          </div>
          ...
          <div class="components-title">
            <svg-icon icon-class="component" />選択的なコンポーネント
          </div>
          ...
          <div class="components-title">
            <svg-icon icon-class="component" /> レイアウトコンポーネント
          </div>
        ...
        <el-row class="center-board-row" :gutter="formConf.gutter">
          <el-form :size="formConf.size" :label-position="formConf.labelPosition" :disabled="formConf.disabled"
            ...
            <div v-show="!drawingList.length" class="empty-info">
              左側からコンポーネントをドラッグまたはクリックしてフォームをデザインします
            </div>
          </el-form>
        </el-row>
      </el-scrollbar>
    ...
    <code-type-dialog v-model="dialogVisible" title="ビルドタイプを選択してください" :showFileName="showFileName" @confirm="generate" />
...
function empty() {
  proxy.$modal.confirm('すべてのコンポーネントをクリアしてもよろしいですか?', 'ヒント', { type: 'warning' }).then(() => {
      idGlobal.value = 100
      drawingList.value = []
...
onMounted(() => {
  clipboard = new ClipboardJS('#copyNode', {
    text: trigger => {
      const codeStr = generateCode()
      ElNotification({ title: '成功', message: 'コードはクリップボードにコピーされているので、貼り付けることができます。', type: 'success' })
      return codeStr
    }
  })
  clipboard.on('error', e => {
    proxy.$modal.msgError('コードのコピーに失敗しました')
  })
})
onUnmounted(() => {
  clipboard.destroy()
