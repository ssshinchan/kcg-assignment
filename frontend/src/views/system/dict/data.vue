<template>
   <div class="app-container">
      <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch">
         ...
         <el-form-item label="辞書タグ" prop="dictLabel">
            <el-input
               v-model="queryParams.dictLabel"
               placeholder="辞書タグを入力してください"
               clearable
               style="width: 200px"
      ...
      <el-table v-loading="loading" :data="dataList" @selection-change="handleSelectionChange">
         ...
         <el-table-column label="辞書エンコーディング" align="center" prop="dictCode" />
         <el-table-column label="辞書タグ" align="center" prop="dictLabel">
            <template #default="scope">
               <span v-if="(scope.row.listClass == '' || scope.row.listClass == 'default') && (scope.row.cssClass == '' || scope.row.cssClass == null)">{{ scope.row.dictLabel }}</span>
               <el-tag v-else :type="scope.row.listClass == 'primary' ? '' : scope.row.listClass" :class="scope.row.cssClass">{{ scope.row.dictLabel }}</el-tag>
            </template>
         </el-table-column>
         <el-table-column label="辞書キー" align="center" prop="dictValue" />
         <el-table-column label="辞書ソート" align="center" prop="dictSort" />
      ...
      <el-dialog :title="title" v-model="open" width="500px" append-to-body>
         <el-form ref="dataRef" :model="form" :rules="rules" label-width="120px">
            <el-form-item label="辞書の種類">
               <el-input v-model="form.dictType" :disabled="true" />
            </el-form-item>
            <el-form-item label="データラベル" prop="dictLabel">
               <el-input v-model="form.dictLabel" placeholder="データラベルを入力してください" />
            </el-form-item>
            <el-form-item label="データキーの値" prop="dictValue">
               <el-input v-model="form.dictValue" placeholder="データキー値を入力してください" />
            </el-form-item>
            <el-form-item label="スタイルプロパティ" prop="cssClass">
               <el-input v-model="form.cssClass" placeholder="スタイル属性を入力してください" />
            </el-form-item>
            <el-form-item label="並べ替えを表示" prop="dictSort">
               <el-input-number v-model="form.dictSort" controls-position="right" :min="0" />
            </el-form-item>
            <el-form-item label="エコースタイル" prop="listClass">
...
const data = reactive({
  ...
  rules: {
    dictLabel: [{ required: true, message: "データラベルを空にすることはできません", trigger: "blur" }],
    dictValue: [{ required: true, message: "データキー値を空にすることはできません", trigger: "blur" }],
    dictSort: [{ required: true, message: "データシーケンスを空にすることはできません", trigger: "blur" }]
  }
})
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('次のようにコード化された辞書を削除してもよろしいですか?"' + dictCodes + '"データ項目?').then(function() {
